...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
	...
	private void drawCellBg(Canvas canvas, int index, int color) {
		...
		canvas.drawRect(left, top, left + surface.cellWidth
				- surface.borderWidth-1, top + surface.cellHeight
				- surface.borderWidth-1, surface.cellBgPaint);
	}
...
//		calendar.setTime(today);
		...
		return time_format.format(today).toString();
	}
	@SuppressLint("SimpleDateFormat")
	public String getSelectYearAndMonth(){
//		calendar.setTime(curDate);
		SimpleDateFormat time_format = new SimpleDateFormat("yyyy-MM-dd");
		System.out.println("select time:"+ time_format.format(selectedStartDate).toString());
		return time_format.format(selectedStartDate).toString();
	}
	//上一月
...
//				}
				selectedStartDate = selectedEndDate = downDate;
				System.out.println("onItemClickListener:" + onItemClickListener+ "selectedStartDate:"+ selectedStartDate);
				//响应监听事件
	...
	private class Surface {
		...
		//public float monthCh angeWidth; // 上一月、下一月按钮宽度
		public float weekHeight; // 显示星期的高度
		public float cellWidth; // 日期方框宽度
		public float cellHeight; // 日期方框高度	
		public float borderWidth;
		public int bgColor = getResources().getColor(R.color.bg_color);
		private int textColor = getResources().getColor(R.color.Calendar_text_color);
		//private int textColorUnimportant = Color.parseColor("#666666");
		private int btnColor = Color.parseColor("#666666");
		private int borderColor = Color.parseColor("#AB15F2");
		public int todayNumberColor = Color.RED;
		public int cellDownColor = Color.parseColor("#CCFFFF");
		public int cellSelectedColor = getResources().getColor(R.color.Calendar_select_color);
		...
		public String[] weekText = { "Su","Mo", "Tu", "We", "Th", "Fr", "Sa"};
